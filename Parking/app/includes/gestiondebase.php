...
function EtablisteAttente()
{

   $req="select id, nom from Inscription  order by id";
...
function obtenirDetailListe($connexion, $id)
{
   $req="select * from Inscription where id='$pseudo'";
   $rsEtab=$connexion->query($req);
   return $rsEtab->fetch(PDO::FETCH_ASSOC);
}
...
function supprimerListe($connexion, $pseudo)
{
   $req="delete from Inscription where pseudo='$pseudo'";
      $connexion->query($req);
}
...
function obtenirnbPersonne($id)
{
  $requete="select distinct id,nombrePersonnes from Groupe, Attribution where
       Attribution.idGroupe=Groupe.id and idEtab='$id'";
  return $requete;
}

function ChambreGroupe($idGroupe)
{
  $req1="select SUM(nombreChambres) FROM attribution,groupe where attribution.idGroupe = groupe.id AND groupe.id= '$idGroupe'";
  return $req1;
}
